...
describe('TickEngine', () => {
  it('creates a world idempotently per seed', () => {
    const { engine } = makeEngine();
    const a = engine.createWorld({ seed: 1337, createdAt: 0 });
    const b = engine.createWorld({ seed: 1337, createdAt: 999_999 });
    expect(a).toBe(b);
  ...
  it('resolves the next tick and records an immutable resolution', async () => {
    ...
    const world = engine.createWorld({ seed: 1337, createdAt: 0 });
    const resolution = await engine.resolveNextTick(world.id, 1000);
    expect(resolution.tick).toBe(1);
    expect(repository.getResolution(world.id, 1)).toBe(resolution);
    const updated = engine.getWorld(world.id);
  ...
  it('a duplicate tick job cannot run two resolvers for the same world/tick', async () => {
    ...
    expect(r1).toBe(r2);
    expect(repository.getResolution(world.id, 1)).toBe(r1);
    expect(engine.getWorld(world.id)?.tick).toBe(1);
  });
  ...
  it('replays an already-resolved tick without re-executing', async () => {
    ...
    expect(engine.getWorld(world.id)?.tick).toBe(1);
    expect(repository.getResolution(world.id, 1)).toBe(first);
  });
