...
description: A deep-space, ledger-calm instrument HUD for a deterministic two-civilization simulation.
colors:
  deep-space: '#0a0e14'
  space-panel: '#121826'
  panel-surface: '#121826'
  panel-border: 'rgba(205, 219, 238, 0.14)'
  star-white: '#dbe5f2'
  space-dust: '#93a2b8'
  ice: '#a8c9f0'
  ice-soft: 'rgba(168, 201, 240, 0.16)'
  ice-deep: '#7fa3d4'
  ice-on: '#0a0e14'
  ice-border: 'rgba(168, 201, 240, 0.45)'
  ice-border-soft: 'rgba(168, 201, 240, 0.35)'
  hover-fill: 'rgba(205, 219, 238, 0.1)'
  hover-fill-strong: 'rgba(205, 219, 238, 0.2)'
...
components:
  button-speed:
    backgroundColor: 'rgba(205, 219, 238, 0.05)'
    textColor: '{colors.star-white}'
    rounded: '{rounded.sm}'
    padding: '7px 10px'
  button-speed-active:
    backgroundColor: '{colors.ice}'
    textColor: '{colors.ice-on}'
    rounded: '{rounded.sm}'
    padding: '7px 10px'
  toggle:
    backgroundColor: 'rgba(205, 219, 238, 0.05)'
    textColor: '{colors.space-dust}'
    rounded: '{rounded.sm}'
    padding: '8px 14px'
  toggle-active:
...
**Creative North Star: "The Deep Archive"**

The interface is a chronicler's ledger suspended in deep space. Near-black surfaces hold ice entries; every number is a witness, every panel a page of the chronicle. The living map below — green pasture, forest, slate water, amber and teal territories — is the world that burns and rebuilds inside the void; the HUD is the archive that records its causes and effects without ever shouting over it.

The archive is void-bound and ledger-calm: a near-black space field, body text in star-white, secondary text in space dust, and a single cold ice used with the discipline of a seal. The deep-space palette is derived from the brand mark (`public/logo.png`): its near-black void and bone cream become the field and the text, its burnt-orange accents retire to the star-white/ice seal. Surfaces are flattened (confirmed direction): panels go solid and opaque, retiring the incumbent translucent glass and backdrop blur; depth comes from tonal separation against the void rather than translucency. Components are refined and restrained — quiet, precise, instrumental — with small uppercase letterspaced labels, monospaced figures, hairline borders, and minimal motion.

**Key Characteristics:**

- Void-bound: near-black space field, star-white text, ice seal
...
The palette is a near-black space neutral field, one ice seal, two faction hues, and the natural terrain gamut of the living map.

### Primary

- **Ice** (#a8c9f0): the only saturated accent on instrument surfaces — a cool star-white. Active speed button, brand milestone tag, terrain-hash readout, citizen carry dots, focus ring, the landing CTA. Rarity is its power — see The Ice Seal Rule.
- **Ice Deep** (#7fa3d4): the deeper cool tone; large fills and display-scale accents where the deeper tone carries.
...
- **Deep Space** (#0a0e14): canvas and app background — the void, a whisper of cool blue.
- **Space Panel** (#121826): solid plate surfaces on the landing page and HUD.
- **Panel Surface** (#121826): floating instruments. Flattening direction: render new surfaces solid and opaque.
- **Star White** (#dbe5f2): body and title text — the cool bone.
- **Space Dust** (#93a2b8): muted text, group labels, the neutral-faction fallback.
- **Panel Border** (rgba(205, 219, 238, 0.14)): the universal hairline, star-tinted.
- **Scorch Red** (#e0605a): danger only — fatal boot errors and severity-2 alerts.
- **Ice On** (#0a0e14): the text and glyph color placed on Ice (active speed button, primary CTA). The void so the ice seal stays legible.
- **Supporting tones:** Ice Border (rgba(168, 201, 240, 0.45)) and Ice Border Soft (rgba(168, 201, 240, 0.35)) for active-state borders; Hover Fill (rgba(205, 219, 238, 0.1)) and Hover Fill Strong (rgba(205, 219, 238, 0.2)) for star-tinted hover surfaces; Shadow Deep (rgba(0, 0, 0, 0.55)) and Shadow Mid (rgba(0, 0, 0, 0.5)) for the ambient panel shadows.
...
**The Ice Seal Rule.** Ice covers ≤10% of any surface. It appears where the player acts or where truth is stamped: the active speed, the hash, the carry. When ice appears twice, the screen is wrong.
...
The void carries the atmosphere. Behind every surface: a deep-space vignette (edges darken), a ruled hairline frame around the viewport, and the deep field — a cold nebula breathing behind the void with a scatter of stars twinkling across it, generated by the landing script (static under `prefers-reduced-motion`).
...
**The Deep Field Rule.** Atmosphere is background paint, never surface. The vignette, the ruled frame, the nebula, and the star scatter sit behind the content, stay still under reduced motion, and never compete with the map or the instruments.
...
- **Default:** star 6% fill, hairline border, star-white text.
- **Active:** Ice fill, void text (#0a0e14), 3px ice-soft halo (`0 0 0 3px` rgba(168, 201, 240, 0.16)).
- **Hover / Focus:** fill lifts to star 10%; focus ring is 2px Ice.### Toggles
- **Shape:** 8px radius, uppercase 12px 600, hairline border.
- **Active:** ice text + ice-soft fill + ice-tinted border (rgba(168, 201, 240, 0.45)).### Chips
- **Style:** full pill, panel surface, mono 12px, space-dust text; the status chip carries an 8px pulsing dot (2s ease) that stays muted until the worker is ready.
...
- **Style:** 8px radius, panel surface, hairline border; code in mono uppercase (muted → ice at severity 1 → scorch red at severity 2).
...
- **Do** keep Ice rare — active state, the hash, the carry; one ice moment per surface.
...
- **Do** keep the brand mark (public/logo.png) on the space field — it is the palette's source and the title, never recolored and never re-typed (see The Brand Mark Rule).### Don't:
...
- **Don't** add saturation beyond the three accents (ice, amber, teal) and the terrain gamut.
